...
for batch_size in batch_sizes:
    ...
    ema_helper.register(online_net)
    ...
    sup_optimizer = optim.Adam(filter(lambda p: p.requires_grad, online_net.parameters()), lr=0.001)
    
    for epoch in range(epochs):
        ema_helper.update(online_net)
        ema_helper.ema(target_net)

        online_net = online_net.train()
        for batch_idx, (images, labels) in enumerate(sup_loader):
            sup_optimizer.zero_grad()
            orig_images, (_, _) = images
            preds = online_net(orig_images, return_logits=True)
